...
with tab3:
    ...
    # 리뷰 작성을 위한 전용 폼 (고유한 key 'review_form' 부여)
    with st.form(key='review_form', clear_on_submit=True):
        ...
        with col_msg:
            comment = st.text_input("리뷰 내용", placeholder="후기를 남겨주세요!")
    ...
    if st.session_state.reviews:
        for r in reversed(st.session_state.reviews):
            st.markdown(f"**{r['name']}** <small style='color:gray;'>({r['date']})</small>")
            st.write(r['msg'])
            st.divider()
    else:
